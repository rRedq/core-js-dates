...
function getNextFriday(date) {
  let currentDay = date.getDay() > 4 ? date.getDay() - 7 : date.getDay();
  let nextFriday = 0;
...
function getCountWeekendsInMonth(month, year) {
  ...
  while (startDay.getMonth() === month - 1) {
    if (startDay.getDay() === 0) count += 1;
    else if (startDay.getDay() === 6) count += 1;
...
function getWeekNumberByDate(date) {
  ...
  while (date.getFullYear() === currentDay.getFullYear()) {
    if (date.getDay() === 0) count += 1;
    date.setDate(date.getDate() - 1);
  }
  if (date.getDay() === 6) count -= 1;
  return count;
}
...
function getNextFridayThe13th(date) {
  ...
  while (flag) {
    if (date.getDay() === 5 && date.getDate() === 13) flag = false;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (flag) {
    if (countWork) {
      ...
      countOff -= 1;
      if (!countOff) countWork = countWorkDays;
    }
    if (start.toDateString() === end.toDateString()) flag = false;
